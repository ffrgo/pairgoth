...
class MacMahonTest {
    ...
    // exports can drop players who never played a real game: Whale (strongest, skips round 1)
    // is never paired and Ghost (weakest) is bye-only, so both must disappear when
    // drop_unplayed=true — and the places of the survivors must be recomputed (Whale tops the
    // full standings on MM base alone, leaving a gap when dropped).
    @Test
    fun `export can drop players who never played a real game`() {
        val tourId = TestAPI.post("/api/tour", BasicTests.aMMTournament).asObject().getInt("id") ?: throw Error("tournament creation failed")
        listOf(
            Json.Object("name" to "Whale", "firstname" to "W", "rating" to 2400, "rank" to 3, "country" to "FR", "club" to "C", "final" to true, "skip" to Json.Array(1)),
        ...
        TestAPI.post("/api/tour/$tourId/pair/1", Json.Array("all")).asArray()
        // asserted via the JSON standings (same filtered list; CSV/EGF/FFG share it)
        ...
        assertEquals(4, full.size, "unfiltered standings list all four")
        assertEquals(1, (full[0] as Json.Object).getInt("place"), "Whale tops the full standings")
        assertEquals("Whale", (full[0] as Json.Object).getString("name"))
        assertEquals(2, filtered.size, "drop_unplayed removes the never-paired and the bye-only players")
        assertEquals(setOf("Alpha", "Beta"), filtered.map { (it as Json.Object).getString("name") }.toSet())
        filtered.forEach {
            assertEquals(1, (it as Json.Object).getInt("place"), "places are recomputed among the survivors (both tied at 1)")
        }
    }
