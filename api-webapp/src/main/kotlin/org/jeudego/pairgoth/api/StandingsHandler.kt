...
object StandingsHandler: PairgothApiHandler {
    override fun get(request: HttpServletRequest, response: HttpServletResponse): Json? {
        ...
        return when (accept) {
            ...
            "application/egf" -> {
                ...
                response.setHeader("Content-Disposition", "attachment; filename=\"$canonicalName.${egfExtension(tournament)}\"")
    ...
    // EGD conventions: the file extension flags even games (.h9) — swiss, or a McMahon at zero
    // correction; a handicap McMahon exports .h<correction>. The HA header keeps the actual
    // correction (h0 when games are even).
    private fun egfExtension(tournament: Tournament<*>): String {
        val correction = tournament.pairing.pairingParams.handicap.correction
        return if (tournament.pairing.type == PairingType.MAC_MAHON && correction > 0) "h$correction" else "h9"
    }

    private fun egfHa(tournament: Tournament<*>): String =
