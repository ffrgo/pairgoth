...
- Incoherent placement criteria are now refused when set: the same criterion twice, two SOS flavours ("Only one of SOS-2, SOS-1, or SOS may be used" — EGF), or two direct-confrontation criteria. Tournaments already carrying such a list still open.
- Separate placement criteria for pairing and for the final standings (Advanced parameters › Player ordering for pairing), which the EGF asks pairing programs to allow: tie-breaks that are sound for making pairings are not necessarily the ones you want to publish. Unset — the default — the draw keeps following the standings criteria.
- Previous order (PREV) and lottery (LOTTERY) placement criteria, completing the EGF's list of tie-breaks. A previous order is fed through the API (`previousOrder` on a player or a team — a qualification rank, a previous tournament's result) and unknown players rank last; the lottery is drawn from the player id, so it is a real draw that never reshuffles between two computations of the standings.
- Board wins placement criterion (BDW): the sum of a team's board results over all rounds, the tie-break the EGF calls "highly meaningful" for team tournaments. New team tournaments use it as their first tie-break; existing ones can add it from the standings tab.
...
- Two players meeting again now get inverse colours, as the EGF tournament system rules require; the colour of a rematch used to be decided by colour balance alone, and by a coin toss when both players were equally balanced. Handicap games are unaffected: the weaker player still takes black.
