...
- "external" auth mode for embedded deployments: a fronting website (e.g. an event site) reverse-proxies pairgoth and injects the authenticated operator's email in a header (`X-Remote-Email`, configurable); operators land in pairgoth already logged in. Tournament visibility is per operator, controlled by ACL symlinks in the store directory (managed by the fronting site, which also allocates tournament ids); access can be granted before a tournament exists — the operator then lands on a creation form prefilled with the provisioned name.
