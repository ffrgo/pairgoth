...
- "external" auth mode for embedded deployments: a fronting website (e.g. an event site) owns the accounts and hands operators over to pairgoth with a short-lived, single-use SSO ticket on `/sso`; sessionless requests bounce to the site's login page and back, so deep links into pairgoth work transparently. Tournament visibility is per operator, controlled by ACL symlinks in the store directory (managed by the fronting site, which also allocates tournament ids); access can be granted before a tournament exists — the operator then lands on a creation form prefilled with the provisioned name. Admin emails (`auth.external.admin`) get unrestricted access — the fronting site's server-to-server channel.
